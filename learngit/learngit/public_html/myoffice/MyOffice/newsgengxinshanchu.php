  ...
  <?php
      ...
      ?>
                <TR>
                  ...
                  <TD class=gridViewItem><a class='gengxin' href=newsgengxin.php?id=<?php echo $info['id'];?>>更新</a></TD>
                  <TD class=gridViewItem><a class='gengxin' href="javascript:if(window.confirm('确定删除该信息？')==true)
             {window.location.href='newsshanchu.php?id=<?php echo $info['id'];?>'}">删除</a></TD>
      ...
      ?>
        ...
        <a href=newsgengxinshanchu.php?page=1>首页</a>
        <a href=newsgengxinshanchu.php?page=<?php if($page!=1){echo $page-1;}else{echo $page;}?>>上一页</a>
        <?php
          for($i=1;$i<=$pages;$i++){
        ?>
              <a href=newsgengxinshanchu.php?page=<?php echo $i ;?>><?php echo $i;?></a>
        <?php
          }
        ?>
        <a href=newsgengxinshanchu.php?page=<?php if($page!=$pages){echo $page+1;}else{echo $pages;}?>>下一页</a>
        <a href=newsgengxinshanchu.php?page=<?php echo $pages;?>>尾页</a>
        <span>跳转到</span><select name='select' onchange="window.location.href=this.value">
          <option value='1' checked>请选择</option>
          <?php
          for($i=1;$i<=$pages;$i++){
          ?>
          <option value=newsgengxinshanchu.php?page=<?php echo $i;?>>第<?php echo $i;?>页</option>
          <?php
        }
          ?>
        </select>
