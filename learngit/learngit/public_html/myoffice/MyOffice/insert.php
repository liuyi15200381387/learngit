  ...
  height=47>
    ...
    vAlign=top align=middle>
        ...
        if(@$_GET['sub']=='确定'){
            $user=$_GET['user'];
            $pass=$_GET['pass'];
            $sql=mysql_query("insert into tb_login(user,pass) values('$user','$pass')");
            if($sql){
            	echo "<script>
				alert('添加成功');
...
<form method='get' action='insert.php' name='myform'>
      ...
      border=1>
            ...
            src="EmployeeMgr.files/bg_users.gif"> </TD>
                  <TD class=gridViewItem><input type='text' name='user' id="users"  onblur='htmlss()'>
                     <font color='red'> <span id="ids"></span></font>
                  </TD>
                  <TD class=gridViewItem><input type='password' name='pass'></TD>
            <TD class=gridViewItem><input type='submit' name='sub' value='确定'></A></TD>
                </TR>
              </TBODY>
            </TABLE>
            </form>
