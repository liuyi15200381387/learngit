...
<DIV>
  ...
  $id=$_GET['id'];
   $_SESSION['ids']=$id;
   $sql=mysql_query("select * from tb_login where id='$id'");
   $info=mysql_fetch_array($sql);
?>
<form method='post' action='gengxinshow.php'>
      ...
      border=1>
            ...
            src="EmployeeMgr.files/bg_users.gif"> </TD>
                  <TD class=gridViewItem><input type='text' name='user' value=<?php echo $info['user'];?>></TD>
                  <TD class=gridViewItem><input type='text' name='pass' value=<?php echo $info['pass'];?>></TD>
                  <TD class=gridViewItem><input type='submit' name='sub' value='确定'></TD>
              </TBODY>
            </TABLE>
            </form>
          </DIV>
